-- Base event-store schema. Append-only stream of recorded domain events plus
-- per-aggregate snapshots used by PdoEventStream to short-circuit replay.
-- The `id` AUTO_INCREMENT primary key serves as the global commit sequence
-- consumed by PdoGlobalEventStream and projection runners.
