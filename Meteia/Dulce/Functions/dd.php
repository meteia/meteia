...
if (!function_exists('jdd')) {
    function jdd(): never
    {
        ...
        $stackTrace = array_slice(array_filter($stackTrace, static fn ($frame) => isset($frame['file'], $frame['line']) && !str_contains($frame['file'], '/vendor/')), 0);
        $commonPrefix = common_prefix_length(array_column($stackTrace, 'file'));
        $stackTrace = array_map(static fn ($frame) => substr($frame['file'], $commonPrefix) . ':' . $frame['line'], $stackTrace);
...
if (!function_exists('hdd')) {
    function hdd(): void
    {
        ...
        $stackTrace = debug_backtrace();
        $stackTrace = array_slice(array_filter($stackTrace, static fn ($frame) => isset($frame['file'], $frame['line']) && preg_match('#/(vendor|DependencyInjection)/#', $frame['file']) === 0), 0);
        $fileNames = array_column($stackTrace, 'file');
        $prefixLength = common_prefix_length($fileNames);
        $stackTrace = array_map(static fn ($frame) => substr($frame['file'], $prefixLength) . ':' . $frame['line'], $stackTrace);
        $stackTrace = array_reverse($stackTrace);

        $data = array_map(dump_value(...), func_get_args());
        array_map(static function ($idx, $data) use ($id): void {
